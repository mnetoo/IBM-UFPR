...
void update_boss(Boss *b, float player_mundo, Player *p) 
{
    ...
    // --- MÁQUINA DE ESTADOS PRINCIPAL DO BOSS ---
    switch(boss_state)
    {
        case BURN:
        case IDLE:
            // Atualiza o sub-estado baseado na vida
            boss_state = (b->vida > 200) ? BURN : IDLE; // Ajuste o valor da vida se necessário
            ...
            {
                boss_shoot_projectile(b); // Dispara um único projétil teleguiado
                b->tempo_tiro = 0;
            }
        ...
        case TELEGRAPH:
            ...
            // O boss para de atacar, dando uma chance para o jogador se preparar.

            if (b->special_attack_phase_timer >= DURACAO_AVISO)
            {
                boss_state = SPECIAL_ATTACK;
                b->special_attack_phase_timer = 0;
                boss_shoot_wall(b); // Dispara a primeira parede de fogo
            }
            break;
        ...
        case SPECIAL_ATTACK:
            ...
            // Dispara rajadas rápidas de paredes de fogo
            if (b->special_attack_phase_timer % 30 == 0) // Dispara uma nova parede a cada meio segundo
            {
                boss_shoot_wall(b);
            }

            // Verifica se a fase de ataque especial terminou
            if (b->special_attack_phase_timer >= DURACAO_ATAQUE_ESPECIAL)
            {
                b->special_attack_cooldown_timer = 0; // Reseta o cooldown principal
    ...
    // --- LÓGICA DE ATUALIZAÇÃO DE TODOS OS PROJÉTEIS ---
    ...
    for (int i = 0; i < MAX_BOSS_PROJECTILES; i++) 
    {
        if (b->projeteis[i].ativo)
        {
            ...
            // Mover o projétil (funciona para ambos os tipos)
 ...
 */
/**
 * @brief - Desenha o Boss e seus projéteis, considerando todos os estados de ataque.
 * 
 * @param b - Ponteiro para a estrutura Boss
 */
void draw_boss(Boss *b) 
{
    ...
    // Determina qual sprite usar baseado no estado atual
    ...
    // Se o estado for BURN, TELEGRAPH, ou SPECIAL_ATTACK, usamos a animação de fogo.
    ...
    // Desenha o boss se um sprite válido foi encontrado
    if (sprite_atual) 
    {
        float escala = 4;
