...
void init_boss(Boss *b, float pos_x) 
{
    ...
    b->patrol_limit_left = pos_x - 300; // Define limite de patrulha 300 pixels à esquerda
    b->patrol_limit_right = pos_x + 300; // e 300 pixels à direita
    b->olhando_para_direita = false;    // Começa olhando para a esquerda (na direção do jogador)
...
void update_boss(Boss *b, float player_mundo, Player *p) 
{
    ...
    //  Movimento de Patrulha (só se move se não estiver no meio de um ataque especial)
    if (boss_state != TELEGRAPH && boss_state != SPECIAL_ATTACK)
    {
        b->x += b->vel_x;
        // Se atingir um dos limites, inverte a velocidade
        if (b->x <= b->patrol_limit_left || b->x >= b->patrol_limit_right)
            b->vel_x *= -1;
    }
...
void draw_boss(Boss *b) 
{
    ...
    if (sprite_atual) 
    {
        float escala = 4;
        // --- NOVA LÓGICA DO FLIP ---
        // Define se o sprite deve ser espelhado baseado na variável `olhando_para_direita`
        int flip = b->olhando_para_direita ? 0 : ALLEGRO_FLIP_HORIZONTAL;
