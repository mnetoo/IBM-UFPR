...
// Cria arco com id de u_id para v_id e adiciona ao grafo G
void cria_direcao(int id, int u_id, int v_id, grafo G) 
{
  vertice u = (vertice) busca_chave(u_id, vertices(G), get_vertice_id);
  vertice v = (vertice) busca_chave(v_id, vertices(G), get_vertice_id);
  ...
  e->id = id;
  e->u = u; // Vértice de origem (source)
  e->v = v; // Vértice de destino (target)

  // Adiciona o arco à lista de arcos do grafo
  empilha(e, arestas(G));

  // Adiciona o arco APENAS à fronteira do vértice de origem u
  empilha(e, fronteira(u)); 
  
  // O campo 'peso' perde o sentido original, mas podemos deixá-lo ou redefiní-lo
  e->peso = 0; // ou podemos usá-lo para outra coisa se necessário
}
